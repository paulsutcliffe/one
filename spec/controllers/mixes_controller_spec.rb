require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe MixesController do

  # This should return the minimal set of attributes required to create a valid
  # Mix. As you add validations to Mix, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) { { "soundcloud_link" => "MyString" } }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # MixesController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET show" do
    it "assigns the requested mix as @mix" do
      mix = Mix.create! valid_attributes
      get :show, {:id => mix.to_param}, valid_session
      assigns(:mix).should eq(mix)
    end
  end

  describe "GET new" do
    it "assigns a new mix as @mix" do
      get :new, {}, valid_session
      assigns(:mix).should be_a_new(Mix)
    end
  end

  describe "GET edit" do
    it "assigns the requested mix as @mix" do
      mix = Mix.create! valid_attributes
      get :edit, {:id => mix.to_param}, valid_session
      assigns(:mix).should eq(mix)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Mix" do
        expect {
          post :create, {:mix => valid_attributes}, valid_session
        }.to change(Mix, :count).by(1)
      end

      it "assigns a newly created mix as @mix" do
        post :create, {:mix => valid_attributes}, valid_session
        assigns(:mix).should be_a(Mix)
        assigns(:mix).should be_persisted
      end

      it "redirects to the created mix" do
        post :create, {:mix => valid_attributes}, valid_session
        response.should redirect_to(Mix.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved mix as @mix" do
        # Trigger the behavior that occurs when invalid params are submitted
        Mix.any_instance.stub(:save).and_return(false)
        post :create, {:mix => { "soundcloud_link" => "invalid value" }}, valid_session
        assigns(:mix).should be_a_new(Mix)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Mix.any_instance.stub(:save).and_return(false)
        post :create, {:mix => { "soundcloud_link" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested mix" do
        mix = Mix.create! valid_attributes
        # Assuming there are no other mixes in the database, this
        # specifies that the Mix created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Mix.any_instance.should_receive(:update_attributes).with({ "soundcloud_link" => "MyString" })
        put :update, {:id => mix.to_param, :mix => { "soundcloud_link" => "MyString" }}, valid_session
      end

      it "assigns the requested mix as @mix" do
        mix = Mix.create! valid_attributes
        put :update, {:id => mix.to_param, :mix => valid_attributes}, valid_session
        assigns(:mix).should eq(mix)
      end

      it "redirects to the mix" do
        mix = Mix.create! valid_attributes
        put :update, {:id => mix.to_param, :mix => valid_attributes}, valid_session
        response.should redirect_to(mix)
      end
    end

    describe "with invalid params" do
      it "assigns the mix as @mix" do
        mix = Mix.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Mix.any_instance.stub(:save).and_return(false)
        put :update, {:id => mix.to_param, :mix => { "soundcloud_link" => "invalid value" }}, valid_session
        assigns(:mix).should eq(mix)
      end

      it "re-renders the 'edit' template" do
        mix = Mix.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Mix.any_instance.stub(:save).and_return(false)
        put :update, {:id => mix.to_param, :mix => { "soundcloud_link" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested mix" do
      mix = Mix.create! valid_attributes
      expect {
        delete :destroy, {:id => mix.to_param}, valid_session
      }.to change(Mix, :count).by(-1)
    end

    it "redirects to the mixes list" do
      mix = Mix.create! valid_attributes
      delete :destroy, {:id => mix.to_param}, valid_session
      response.should redirect_to(mixes_url)
    end
  end

end
